...
bool WIN_InitInstance()
{
    ...
    WIN_hWnd = CreateWindowEx(dwExStyle, "WIN_WindowClass", "Game Window", dwStyle, 0, 0, r.right - r.left,
                              r.bottom - r.top, NULL, NULL, WIN_hInst, NULL);
...
int APIENTRY WinMain(_In_ HINSTANCE hI, _In_opt_ HINSTANCE hPrevI, _In_ LPSTR lpCmdLine, _In_ int nCS)
{
    WIN_hInst = hI;
    WIN_nCmdShow = nCS;
    if (!WIN_RegisterClass())
        return -1;
    if (SCR_FULLSCREEN)
    {
        if (!WIN_SetFullScreen())
            return -1;
    }
    if (!WIN_InitInstance())
        return -1;

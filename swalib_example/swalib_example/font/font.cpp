...
char FONT_Def[FONT_ROWS][FONT_COLS + 1] = {
    {" !\"~*%-'"}, {"   +, ./"}, {"01234567"}, {"89:;aib?"}, {"*ABCDEFG"}, {"HIJKLMNO"}, {"PQRSTUVW"}, {"XYZ     "},
};
...
void FONT_DrawChar(const vec2 &p0, const vec2 &p1, unsigned char ch)
{
    if ((int)ch < MAX_FONT_CHARDEFS && fontchardefs[ch].ch == ch)
    {
        ...
        glBegin(GL_QUADS);
        glTexCoord2d(fontchardefs[ch].p0.x, fontchardefs[ch].p0.y);
        ...
        glTexCoord2d(maxCoordX, fontchardefs[ch].p0.y);
        glVertex2f(p1.x, p0.y);
        glTexCoord2d(maxCoordX, maxCoordY);
        glVertex2f(p1.x, p1.y);
        glTexCoord2d(fontchardefs[ch].p0.x, maxCoordY);

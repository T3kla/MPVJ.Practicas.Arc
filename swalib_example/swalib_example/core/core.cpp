...
static GLuint CreateOpenGLTextureFromData(byte *pixdata, int width, int height, bool wrap, bool bgra = false)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, abs((int)height), 0, (bgra ? GL_BGRA_EXT : GL_RGBA),
    ...
    if (fd != -1)
    {
            ...
            hdr.format[0] == 'W' && hdr.format[1] == 'A' && hdr.format[2] == 'V' && hdr.format[3] == 'E')
        {
            ...
            for (;;)
            {
                ...
                if (chunkhdr.subChunkID[0] == 'f' && chunkhdr.subChunkID[1] == 'm' && chunkhdr.subChunkID[2] == 't' &&
                    chunkhdr.subChunkID[3] == ' ')
                {
                    read(fd, &fmt, sizeof(fmt));
                    lseek(fd, ((1 + chunkdatasize) & -2) - sizeof(fmt), SEEK_CUR); // Skip to next chunk
                }
                else if (chunkhdr.subChunkID[0] == 'd' && chunkhdr.subChunkID[1] == 'a' &&
                         chunkhdr.subChunkID[2] == 't' && chunkhdr.subChunkID[3] == 'a')
                {
                    ...
                    {
                        if (READ_LE_WORD(fmt.bitsPerSample) == 8)
                            al_format = AL_FORMAT_MONO8;
                        else if (READ_LE_WORD(fmt.bitsPerSample) == 16)
